...
public interface CamperCanReportRepository extends JpaRepository<CamperCanReport, Long> {
    ...
    @Query(value = "SELECT * FROM camper_can_report WHERE camper_id = ?1 AND caterer_id = ?2 AND reservation_end_date <= ?3", nativeQuery = true)
    CamperCanReport findCamperCanReport(Long camperId, Long catererId, LocalDate today);
}
